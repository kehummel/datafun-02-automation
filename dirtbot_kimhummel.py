...
def create_folders_from_list(folder_list: list) -> None:
    ...
    for name in folder_list
        name_path = ROOT_DIR / str(name).lower
...
def create_standardized_folders(folder_list: list, to_lowercase: bool = False, remove_spaces: bool = False) -> None:
    ...
    for name in folder_list(str)
        stand_name = name
        if to_lowercase: 
            name = folder_list.lower 
        if remove_spaces:
            name = folder_list.replace(" ", "")
        folder_path = ROOT_DIR / folder
        folder_path.mkdir(exist_ok=True)
        logger.info(f"Created standardizations of the folder: {stand_name}")
...
def main() -> None:
    ...
    logger.info(f"Byline: {datafun_01_utils.get_byline()}")
